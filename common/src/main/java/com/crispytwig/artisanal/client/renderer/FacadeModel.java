...
public class FacadeModel implements BakedModel {
    ...
    private record ModelKey(BakedModel parent, BlockState state, Map<Direction, Block> facades) {
    }
    ...
    public static BakedModel wrap(BakedModel model, BlockState state, BlockPos pos) {
        ...
        Map<Direction, Block> facades = FacadeClient.facades(pos);
        if (facades == null) {
            return model;
        }
    ...
    private FacadeModel(BakedModel parent, BlockState state, Map<Direction, Block> facades) {
        ...
        for (BakedQuad quad : parent.getQuads(state, null, random)) {
            Block block = facades.get(quad.getDirection());
    ...
    public @NotNull List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction direction, @NotNull RandomSource random) {
        ...
        List<BakedQuad> facade = faces.get(direction);
        return facade != null ? facade : parent.getQuads(state, direction, random);
    }
