...
import com.crispytwig.artisanal.block.WeatheringLayerBlock;
import com.crispytwig.artisanal.data.LayerData;
...
public final class ModLayers {
    ...
    public static void register() {
        ...
        for (Map.Entry<ResourceKey<Block>, Block> entry : BuiltInRegistries.BLOCK.entrySet()) {
            ...
            if (slab instanceof SlabBlock
                ...
                found.put(Artisanal.location(layerName(slabId)), slabId);
            }
        }
    ...
    private static Map<ResourceLocation, ResourceLocation> waxing() {
        ...
        LAYERS.forEach((layerId, layer) -> {
            if (layerId.getPath().contains(WAXED_PREFIX)) {
                ResourceLocation unwaxed = Artisanal.location(layerId.getPath().replace(WAXED_PREFIX, ""));
    ...
    private static ResourceLocation previousStage(ResourceLocation layerId) {
        ...
        if (path.contains("oxidized")) {
            return Artisanal.location(path.replace("oxidized", "weathered"));
        }
        if (path.contains("weathered")) {
            return Artisanal.location(path.replace("weathered", "exposed"));
        }
        if (path.contains("exposed_")) {
            return Artisanal.location(path.replace("exposed_", ""));
        }
        return null;
    }
