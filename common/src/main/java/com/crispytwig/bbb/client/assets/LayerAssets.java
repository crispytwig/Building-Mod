...
public final class LayerAssets {
    ...
    public static void generate(ResourceManager manager) {
        BuildingButBetterPack.INSTANCE.clear(PackType.CLIENT_RESOURCES);
        CACHE.clear();

        JsonObject lang = new JsonObject();
        ModLayers.parents().forEach((layerId, slabId) -> {
            lang.addProperty("block." + BuildingButBetter.MOD_ID + "." + layerId.getPath().replace('/', '.'), title(layerId));
    ...
    private static void write(String path, JsonObject json) {
        BuildingButBetterPack.INSTANCE.add(PackType.CLIENT_RESOURCES, BuildingButBetter.location(path + ".json"), json);
    }
    ...
    private static String title(ResourceLocation layerId) {
        ...
        return BuildingButBetter.titleCase(path.substring(path.indexOf('/') + 1));
    }
}
